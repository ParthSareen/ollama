...
1. For each environment variable, call `launchctl setenv`.
   ...
   ```bash
   launchctl setenv OLLAMA_HOST "0.0.0.0:11433"
   ```
...
docker run -d -e HTTPS_PROXY=https://my.proxy.example.com -p 11433:11433 ollama-with-ca
```
...
Ollama binds 127.0.0.1 port 11433 by default. Change the bind address with the `OLLAMA_HOST` environment variable.
...
server {
    ...
    location / {
        proxy_pass http://localhost:11433;
        proxy_set_header Host localhost:11433;
    }
}
```
...
```shell
ngrok http 11433 --host-header="localhost:11433"
```
...
```shell
cloudflared tunnel --url http://localhost:11433 --http-host-header="localhost:11433"
```
...
```shell
curl http://localhost:11433/api/generate -d '{"model": "mistral"}'
```

To use the chat completions endpoint, use:

```shell
curl http://localhost:11433/api/chat -d '{"model": "mistral"}'
```
...
```shell
curl http://localhost:11433/api/generate -d '{"model": "llama3.2", "keep_alive": -1}'
```

To unload the model and free up memory use:

```shell
curl http://localhost:11433/api/generate -d '{"model": "llama3.2", "keep_alive": 0}'
```
