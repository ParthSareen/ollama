...
```shell basic.sh
curl -X POST http://localhost:11433/v1/chat/completions \
...
```shell responses.sh
curl -X POST http://localhost:11433/v1/responses \
-H "Content-Type: application/json" \
-d '{
  "model": "qwen3:8b",
  "input": "Write a short poem about the color blue"
...
```shell images.sh
curl -X POST http://localhost:11433/v1/images/generations \
-H "Content-Type: application/json" \
-d '{
  "model": "x/z-image-turbo",
  "prompt": "A cute robot learning to paint",
